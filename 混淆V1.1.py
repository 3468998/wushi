    ...
    def update_preview(self):
        ...
        try:
            ...
            if not password or not key:
                ...
                self.preview_text.insert(tk.END, "请输入密码和密钥以生成WebShell")
                self.update_line_numbers()
                self.status_var.set("等待输入密码和密钥")
                return
...
?>"""
            ...
            # 更新状态
            self.status_var.set(f"WebShell已生成 - 代码长度: {len(final_php)} 字符")
    ...
    def clear_preview(self):
        ...
        # 重新绑定自动更新
        self.password_var.trace_add("write", lambda *args: self.update_preview())
        self.key_var.trace_add("write", lambda *args: self.update_preview())
